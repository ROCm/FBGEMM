...
DEVICE_INLINE __device__ float wave_reduce_add(float value, int N) {
    // static_assert(__AMDGCN_WAVEFRONT_SIZE == 64, "Wavefront size must be 64");
...
template <
    ...
    bool kUseVecBlocking
>
DEVICE_INLINE void split_rowwise_adagrad_table_update_kernel_maxD_128(
    pta::PackedTensorAccessor64<emb_t, 1, at::RestrictPtrTraits>& dev_weights,
    pta::PackedTensorAccessor64<emb_t, 1, at::RestrictPtrTraits>& uvm_weights,
    pta::PackedTensorAccessor64<cache_t, 2, at::RestrictPtrTraits>& lxu_cache_weights,
    const pta::PackedTensorAccessor32<int32_t, 1, at::RestrictPtrTraits>& weights_placements,
    ...
    //     GROUP_REDUCE_ALL_SUM(g_local_sum_square, at::acc_type<cache_t, true>) / D;
    const at::acc_type<cache_t, true> g_avg_square =
         wave_reduce_add(g_local_sum_square, kThreadGroupSize) / D;
    ...
    if (max_norm > 0.0) {
        ...
        //     sqrtf(GROUP_REDUCE_ALL_SUM(weight_sum_square, at::acc_type<cache_t, true>));
        const at::acc_type<cache_t, true> weight_norm =
            sqrtf(wave_reduce_add(weight_sum_square, kThreadGroupSize));
...
template <
    ...
    bool kUseVecBlocking
>
DEVICE_INLINE void split_rowwise_adagrad_table_update_kernel_device_maxD_128(
    pta::PackedTensorAccessor64<emb_t, 1, at::RestrictPtrTraits>& dev_weights,
    pta::PackedTensorAccessor64<emb_t, 1, at::RestrictPtrTraits>& uvm_weights,
    pta::PackedTensorAccessor64<cache_t, 2, at::RestrictPtrTraits>& lxu_cache_weights,
    // const pta::PackedTensorAccessor32<int32_t, 1, at::RestrictPtrTraits>& weights_placements,
    ...
    //       printf("debug line:%d threadIdx: (%d, %d) run_id: %d grad_sum[0]: %f reduce_sum: %f multiplier: %f correction: %f kThreadGroupSize: %d idx: %d momentum1: %f momentum1_offset: %lld\n", __LINE__, threadIdx.y, threadIdx.x, run_id, grad_sum[0].acc.x, wave_reduce_add(g_local_sum_square, kThreadGroupSize), multiplier, correction, kThreadGroupSize, idx, momentum1[idx], momentum1_offset);
    // }
