...
DEVICE_INLINE void compute_grad_sum_unweighted_vbe(
    ...
    auto num_lanes_per_subwarp = kAlignD / VEC_WIDTH;
    auto num_subwarp = kThreadGroupSize / num_lanes_per_subwarp ;
    auto subwarp_id = threadIdx.x / num_lanes_per_subwarp;
    auto lane_id = threadIdx.x % num_lanes_per_subwarp;
    ...
    for (int32_t vec_start = 0;
        ...
        for (int32_t sl = sl_start; sl < sl_end; sl += kThreadGroupSize) {
            ...
            for (; j < kThreadGroupSize; j += num_id_per_block_x) {
                ...
                #pragma unroll kFixedMaxVecsPerThread
                for (int32_t vec = 0; vec < kFixedMaxVecsPerThread && (((vec + vec_start) * kThreadGroupSize + lane_id) * VEC_WIDTH) < D; ++vec) {
                    const int32_t d = (((vec + vec_start) * kThreadGroupSize + lane_id) * VEC_WIDTH);
        ...
        #pragma unroll kFixedMaxVecsPerThread
        for (int32_t vec = 0; vec < kFixedMaxVecsPerThread; vec++) {
            for (auto offset = kThreadGroupSize / 2; offset >= num_lanes_per_subwarp; offset >>= 1) {
                grad_sum[vec].acc.x += __shfl_down_sync(UINT64_MAX, grad_sum[vec].acc.x, offset);
                grad_sum[vec].acc.y += __shfl_down_sync(UINT64_MAX, grad_sum[vec].acc.y, offset);
                grad_sum[vec].acc.z += __shfl_down_sync(UINT64_MAX, grad_sum[vec].acc.z, offset);
                grad_sum[vec].acc.w += __shfl_down_sync(UINT64_MAX, grad_sum[vec].acc.w, offset);
            }
        }
