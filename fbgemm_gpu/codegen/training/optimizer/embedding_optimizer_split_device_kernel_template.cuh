...
DEVICE_INLINE void split_rowwise_adagrad_table_update_kernel_device(
    ...
    const int64_t idx,
    const float global_weight_decay,
    const uint32_t _shfl_sync_mask,
    const int32_t max_vecs_per_thread,
    // pta::PackedTensorAccessor64<at::acc_type<cache_t, true>, 1, at::RestrictPtrTraits>& momentum1_dev,
    // pta::PackedTensorAccessor64<at::acc_type<cache_t, true>, 1, at::RestrictPtrTraits>& momentum1_uvm,
    // pta::PackedTensorAccessor32<int32_t, 1, at::RestrictPtrTraits>& momentum1_placements,
    ...
    //     GROUP_REDUCE_ALL_SUM(g_local_sum_square, at::acc_type<cache_t, true>) / D;
    const at::acc_type<cache_t, true> g_avg_square =
         subwarp_reduce_add<kThreadGroupSize>(g_local_sum_square) / D;
    ...
    if (threadIdx.x == 0) {	
        ...
        if (enable_optimizer_offloading) {
            ...
            momentum1[idx] = new_sum_square_grads;
            // new_sum_square_grads += atomicAdd(&momentum1[idx], new_sum_square_grads);
        }
    ...
    multiplier = SUBWARP_SHFL_SYNC(multiplier, 0);
    correction = SUBWARP_SHFL_SYNC(correction, 0);
    // if (threadIdx.x == 0 && threadIdx.y <= 1 && blockIdx.x == 0 && blockIdx.y == 0 && false) {
    //       printf("debug line:%d threadIdx: (%d, %d) run_id: %d grad_sum[0]: %f reduce_sum: %f multiplier: %f correction: %f kThreadGroupSize: %d idx: %d momentum1: %f momentum1_offset: %lld\n", __LINE__, threadIdx.y, threadIdx.x, run_id, grad_sum[0].acc.x, subwarp_reduce_add(g_local_sum_square, kThreadGroupSize), multiplier, correction, kThreadGroupSize, idx, momentum1[idx], momentum1_offset);
    // }
    ...
    if (max_norm > 0.0) {
        ...
        //     sqrtf(GROUP_REDUCE_ALL_SUM(weight_sum_square, at::acc_type<cache_t, true>));
        const at::acc_type<cache_t, true> weight_norm =
            sqrtf(subwarp_reduce_add<kThreadGroupSize>(weight_sum_square));
