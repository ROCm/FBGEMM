...
static torch::autograd::variable_list group_index_select_dim0_forward_impl_gpu(
  ...
  for (const auto i : c10::irange(group_size)) {
    ...
    if (num_cols_ < 0 && num_cols_ >= unroll_factor) {
...
static torch::autograd::variable_list group_index_select_dim0_backward_impl_gpu(
  ...
  std::cout << "\n=== DEBUG BACKWARD PASS INPUTS ===" << std::endl;
  std::cout << "Total all_inputs.size(): " << all_inputs.size() << std::endl;

  // Print metadata for each input tensor
  for (int i = 0; i < all_inputs.size(); ++i) {
      const auto& t = all_inputs[i];
      std::cout << "Input [" << i << "]: ";
      if (t.defined()) {
          ...
          std::cout << "UNDEFINED (None)";
      }
      std::cout << std::endl;
  }
  std::cout << "==================================\n" << std::endl;

  // Layout: [Grads(G) | AuxGrads(2) | Indices(G) | ArgsS, ArgsL, SavedS, SavedL, FwdInput(5)]
  // Total = 2*G + 7
  TORCH_CHECK(all_inputs.size() > 7);

  // all_input size =  group_size * 2 (from grads, indices)
  // + 2 args_tensors + 2 gradient vectors generated by PyTorch corresponding to the args_tensors
  // + 2 saved_data + 1 first input
  const int64_t group_size = (all_inputs.size() - 7) / 2;

  const size_t n = all_inputs.size();
  const Tensor& fwd_input = all_inputs[n - 1];
  const Tensor& saved_data_large = all_inputs[n - 2];
  const Tensor& saved_data_small = all_inputs[n - 3];
  // const Tensor& args_tensor_large = all_inputs[n - 4];
  // const Tensor& args_tensor_small = all_inputs[n - 5];
  ...
  // Skip G grads + 2 aux grads. Start indices at G + 2.
  auto indices_group = std::vector<Tensor>(
      all_inputs.cbegin() + group_size + 2, all_inputs.cbegin() + 2 + 2 * group_size);
  ...
  for (int i = 0; i < group_size; i++) {
    ...
    if(grad_output_group[i].size(1) < 0) {
